...
##############################################################################################################################################################################
    ...
    make_background:
        ...
        sw $zero, 0($t0) 
        la $t0, DealerOverFlow
        sw $zero, 0($t0)  # reset all of our bust values and ace count values for the new hand
        ...
        initial_deal:
            ...
            li $t1, 101505
            bne $v0, $t1, creator_special  # give myself infinite chips
                la $t0, Bank
                li $t1, 10000000
                sw $t1, 0($t0)
                li $v0, 1
            ...
            ble $v0, $zero wager_ifelse  # if the wager is <= 0, it cant happen
            bgt $v0, $t0, wager_ifelse  # if wager is more than the bank, it cant happen
            
            la $t0, Wager
            sw $v0, 0($t0)  # store the wager amount from input
            j bypass_wager_ifelse
...
##############################################################################################################################################################################
    hit_protocol:
        ...
        la $s4, UserAceCount1
        li $s1, 127
        li $s0, 144  # we dont include the first hit in the loop
        hit_loop:
            li $t8, 1000000
        ...
        hit_ace_check:
            ...
            addi $t4, $t4, -1
            addi $t5, $t5, -10  # ace low, decrement ace count value
            sw $t4, 0($s4)
            sw $t5, 0($s5)
    ...
    stand_protocol:
        ...
        stand_ace_check:
            ...
            addi $t4, $t4, -1
            addi $t5, $t5, -10  # remove ten, take ace from the count
            sw $t4, 0($s4)
            sw $t5, 0($s5)  # save to original values
            j stand_ace_check
    ...
    double_protocol:
        ...
        double_ace_check:
            ...
            sw $t4, 0($s4)
            sw $t5, 0($s5)  # update user bust value and ace count
    ...
    dealer_protocol:
        ...
        dealer_decision_check:
            ...
            dealer_ace_check:
                ...
                addi $t4, $t4, -1  # take ace from count
                addi $t5, $t5, -10  # ace low
                sw $t4, 0($s4)
                sw $t5, 0($s5)  # update dealer bust value and ace count value
                li $t0, 17
                bge $t5, $t0, dealer_decision_check  # if dealer has more than 16, go to decision
                jr $ra  # if dealer now has less than 17, return to draw another card
##############################################################################################################################################################################
